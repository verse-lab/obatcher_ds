...
assert (IntTreap.verify_heap_property_aux at1.root);;
assert (IntTreap.verify_heap_property_aux at2.root);;
let () = for i = 0 to pivot do
  if ref_array_2.(i) != -1 then
    assert (IntTreap.search i at1 = Some ref_array_2.(i))
done;;
let () = for i = pivot + 1 to max_key - 1 do
  if ref_array_2.(i) != -1 then
    assert (IntTreap.search i at2 = Some ref_array_2.(i))
done;;
Printf.printf "Verification time for Treaps: %fs\n" (Sys.time() -. st);;

let st = Sys.time();;
let mn = IntTreap.new_node pivot 232;;
let jt = IntTreap.join at1 mn at2;;
Printf.printf "Join time for Treaps: %fs\n" (Sys.time() -. st);;

let st = Sys.time();;
assert (IntTreap.verify_heap_property_aux jt.root);;
let () = for i = 0 to max_key - 1 do
  if ref_array_2.(i) != -1 then
    assert (IntTreap.search i jt = Some ref_array_2.(i))
done;;
...
let st = Sys.time();;
assert (IntTreap.verify_heap_property_aux at.root);;
let () = for i = 1 to max_key - 1 do
  if ref_array_2.(i) != -1 then
    assert (IntTreap.search i at = Some ref_array_2.(i))
done;;
...
let (lt, mn, rt) = IntTreap.split at (split_pt);;
Printf.printf "Split time for Treap: %fs\n" (Sys.time() -. st);;

let st = Sys.time();;
assert (IntTreap.verify_heap_property_aux lt.root);;
assert (IntTreap.verify_heap_property_aux rt.root);;
let () = for i = 1 to split_pt - 1 do
  if ref_array_2.(i) != -1 then
    assert (IntTreap.search i lt = Some ref_array_2.(i))
done;;
let () = for i = split_pt + 1 to max_key - 1 do
  if ref_array_2.(i) != -1 then
    assert (IntTreap.search i rt = Some ref_array_2.(i))
done;;
assert (match mn with
| IntTreap.Leaf -> ref_array_2.(split_pt) = -1
| IntTreap.Node n' -> n'.nval = ref_array_2.(split_pt));;
...
let st = Sys.time();;
assert (IntTreap.verify_heap_property_aux at3.root);;
let num_found = ref 0;;
let () = for i = 0 to max_key - 1 do
  if ref_array_3.(i) != -1 then
    (assert (IntTreap.search i at3 = Some ref_array_3.(i));
    num_found := !num_found + 1)
  else
    assert (IntTreap.search i at3 = None)
done;;
