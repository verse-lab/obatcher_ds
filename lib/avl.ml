...
module Prebatch (V: Map.OrderedType) = struct
  ...
  let peek_tree (t: 'a S.tree) = t.root
  ...
  let merge_three_nodes (nl: 'a S.node) (n: 'a S.node) (nr: 'a S.node) =
    ...
    | Node _ ->
      ...
      S.set_height n ((max (height nl) (height nr)) + 1)

  let rec join_right (tl: 'a S.tree) k (tr: 'a S.tree) =
  ...
  let rec join_left (tl: 'a S.tree) n (tr: 'a S.tree) =
  ...
  let join (tl: 'a S.tree) (n: 'a S.node) (tr: 'a S.tree) =
  ...
  let set_root_node (n: 'a S.node) (t: 'a S.tree) = t.root <- n
