...
module HashXfastTabl = struct
  ...
  let create n_levels = {
    max_lvl = n_levels - 1;
    arr = Array.init n_levels (fun i -> Hashtbl.create (Int.shift_left 1 (i + 1)))
  }
...
module Sequential = struct
  ...
  module T = Xfast.ArrXfastTabl
  module Xfast = Xfast.Sequential(T)
  ...
  let init int_size =
    ...
    let t = {
      ...
      rb_layer = Hashtbl.create (Int.shift_left 1 (int_size / 2))
    } in
  ...
  let flatten t =
    let idx_arr = List.rev @@ T.flatten t.xfast_layer.tables in
    let res = ref [] in
    List.iter
      (fun idx ->
