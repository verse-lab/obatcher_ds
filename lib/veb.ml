...
module Sequential = struct
  ...
  let update_node t =
    ...
    Array.iteri
      (fun i ct ->
        if ct.min = None then
          delete (Option.get t.summary) i
        else begin
          insert (Option.get t.summary) i; (* Update summary accordingly *)
        ...
        if ct.max != None then (cmax_idx := Some i)) *)
      clusters;

    (* Replace minimum and maximum in current node if needed *)
    (* Process:
       - Check if a minimum exists in the clusters; retrieve its value.
       - Extract the minimum from the clusters
       - 
    *)
    ...
    (* if !cmin_idx != None then begin
      let cmin_idx = Option.get !cmin_idx
      and cmax_idx = Option.get !cmax_idx in
      let cmin = Option.get clusters.(cmin_idx).min
      and cmax = Option.get clusters.(cmax_idx).max in 
      let ncmin = index t cmin_idx cmin
      and ncmax = index t cmax_idx cmax in
      if t.min = None || ncmin <= Option.get t.min then begin
        delete clusters.(cmin_idx) cmin;
        if clusters.(cmin_idx).min = None then
          delete (Option.get t.summary) cmin_idx;
        let tmp_min = t.min in
        t.min <- Some ncmin;
        if tmp_min != None then insert t (low t @@ Option.get tmp_min)
      end;
      if cmax != cmin && (t.max = None || Option.get t.max < ncmax) then t.max <- Some ncmax
    end *)
  ...
  let par_insert_aux_4 op_threshold ~pool t ~inserts ~range:(rstart, rstop) =
    ...
    if n <= 0 then ()
      ...
      (
        (* Printf.printf "Running sequentially. Inserting %d elements.\n" n; *)
      for i = rstart to rstop - 1 do
        (* Printf.printf "Inserting %d elements sequentially\n" inserts.(i); *)
        Sequential.insert t inserts.(i);
        (* Printf.printf "We did it!\n" *)
      done)
    else begin
      ...
      (* Printf.printf "Rstart to rstop: %d\n" (rstop - rstart);
      Printf.printf "Npivots: %d\n" (Array.length pivots_arr);

      Printf.printf "Inserting: [";
      for i = rstart to rstop - 1 do
        Printf.printf "%d, " inserts.(i)
      done;
      Printf.printf "]\n";
      Printf.printf "Pivots: [";
      for i = 0 to Array.length pivots_arr - 1 do
        Printf.printf "%d, " pivots_arr.(i)
      done;
      Printf.printf "]\n"; *)
      ...
      for i = 0 to Array.length ranges - 1 do
        Printf.printf "(%d, %d), " (fst ranges.(i)) (snd ranges.(i))
      done;
      Printf.printf "]\n"; *)
      ...
      Domainslib.Task.parallel_for pool ~start:0 ~finish:(Array.length ranges - 1) ~chunk_size:1
        ~body:(fun i ->
          for j = fst ranges.(i) to snd ranges.(i) - 1 do 
            Sequential.insert
              clusters.(Sequential.high t inserts.(j))
              (Sequential.low t inserts.(j))
          done);
      Sequential.update_node t
    end
