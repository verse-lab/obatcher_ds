...
module Sequential = struct
  ...
  let cleanup (t: t) (test_spec: test_spec) = 
    if test_spec.args.should_validate then begin
      (* if BatchedIntAvltree.Sequential.num_nodes t <> Array.length test_spec.insert_elements + Array.length test_spec.initial_elements
      then Format.ksprintf failwith "Inserted %d elements, but found only %d in the tree"
             ...
             t.IntAvltree.Sequential.root.no_elements;
      let avltree_flattened = IntAvltree.flatten t.root |> Array.of_seq in
      let all_elements = Array.concat [test_spec.insert_elements; test_spec.initial_elements] in
      Array.sort Int.compare all_elements;
      if Array.length avltree_flattened <> Array.length all_elements then
        Format.ksprintf failwith "length of flattened avltree (%d) did not match inserts (%d) (no_elements=%d)"
          (Array.length avltree_flattened) (Array.length all_elements) (t.root.no_elements);

      for i = 0 to Array.length avltree_flattened - 1 do
        if fst avltree_flattened.(i) <> all_elements.(i) then
          Format.ksprintf failwith "element %d of the avltree was expected to be %d, but got %d" i
            all_elements.(i) (fst avltree_flattened.(i));
      done; *)
