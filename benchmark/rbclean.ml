...
let generic_run test_spec f =
  let old_search_threshold = !IntRbtreeSplitJoin.search_op_threshold in
  let old_insert_threshold = !IntRbtreeSplitJoin.insert_op_threshold in
  (match test_spec.args.search_threshold with None -> () | Some st ->
    IntRbtreeSplitJoin.search_op_threshold := st);
  (match test_spec.args.insert_threshold with None -> () | Some it ->
    IntRbtreeSplitJoin.insert_op_threshold := it);
  let res = f () in
  IntRbtreeSplitJoin.search_op_threshold := old_search_threshold;
  IntRbtreeSplitJoin.insert_op_threshold := old_insert_threshold;
  res
...
module Sequential = struct
  ...
  let init _pool test_spec =
    generic_init test_spec (fun initial_elements ->
      let tree = IntRbtree.init () in
      Array.iter (fun i -> IntRbtree.insert i () tree)
        initial_elements;
      tree
    )
...
module CoarseGrained = struct
    ...
    Domainslib.Task.parallel_for pool ~chunk_size:1
      ...
      ~body:(fun i ->
          ...
          Fun.protect ~finally:(fun () -> Mutex.unlock t.mutex) (fun () ->
              ...
              then IntRbtree.insert test_spec.insert_elements.(i) () t.tree
              else ignore (IntRbtree.search
  ...
  let cleanup (t: t) (test_spec: test_spec) =
    if test_spec.args.should_validate then begin
      Array.iter (fun elt ->
        match IntRbtree.search elt t.tree with
        | Some _ -> ()
        | None -> Format.ksprintf failwith "Could not find inserted element %d in tree" elt
      ) test_spec.insert_elements
    end
...
module Batched = struct
  ...
  let init pool test_spec =
    generic_init test_spec (fun initial_elements ->
      ...
      let exposed_tree = BatchedIntRbtree.unsafe_get_internal_data tree in
      Array.iter (fun i -> IntRbtree.insert i () exposed_tree)
        initial_elements;
      BatchedIntRbtree.restart_batcher_timer tree;
      tree)
  ...
  let cleanup (t: t) (test_spec: test_spec) =
    if test_spec.args.should_validate then begin
      ...
      let num_nodes = IntRbtree.num_nodes t in
      ...
      num_nodes;
      let btree_flattened = IntRbtree.flatten t |> Array.of_list in
...
module ExplicitlyBatched = struct
  ...
  let run pool (tree: t) test_spec =
    ...
    if Array.length test_spec.insert_elements > 0 then
      IntRbtreeSplitJoin.par_insert ~pool tree test_spec.insert_elements;
    if Array.length test_spec.spec.search_elements > 0 then
      ignore @@ IntRbtreeSplitJoin.par_search ~pool tree test_spec.search_elements
