...
let generic_run test_spec f =
  let old_search_threshold = !IntAvltree.Sequential.search_op_threshold in
  let old_insert_threshold = !IntAvltree.Sequential.insert_op_threshold in
  let old_search_type = !IntAvltree.Sequential.search_type in
  let old_insert_type = !IntAvltree.Sequential.insert_type in
  (match test_spec.args.search_threshold with None -> () | Some st ->
    IntAvltree.Sequential.search_op_threshold := st);
  (match test_spec.args.insert_threshold with None -> () | Some it ->
    IntAvltree.Sequential.insert_op_threshold := it);
  (match test_spec.args.search_type with None -> () | Some it ->
    IntAvltree.Sequential.search_type := it);
  (match test_spec.args.insert_type with None -> () | Some it ->
    IntAvltree.Sequential.insert_type := it);
  let res = f () in
  IntAvltree.Sequential.search_op_threshold := old_search_threshold;
  IntAvltree.Sequential.insert_op_threshold := old_insert_threshold;
  IntAvltree.Sequential.search_type := old_search_type;
  IntAvltree.Sequential.insert_type := old_insert_type;
  res
...
module Sequential = struct
  ...
  let run _pool t test_spec =
    ...
    Array.iter (fun i ->
        IntAvltree.Sequential.insert i () t
      ) test_spec.insert_elements;
    Array.iter (fun i ->
        ignore @@ IntAvltree.Sequential.search i t
...
module CoarseGrained = struct
    ...
    Domainslib.Task.parallel_for pool ~chunk_size:1
      ...
      ~body:(fun i ->
          ...
          Fun.protect ~finally:(fun () -> Mutex.unlock t.mutex) (fun () ->
              ...
              then IntAvltree.Sequential.insert test_spec.insert_elements.(i) () t.tree
              else ignore (IntAvltree.Sequential.search
  ...
  let cleanup (t: t) (test_spec: test_spec) =
    if test_spec.args.should_validate then begin
      Array.iter (fun elt ->
        match IntAvltree.Sequential.search elt t.tree with
        | Some _ -> ()
        | None -> Format.ksprintf failwith "Could not find inserted element %d in tree" elt
      ) test_spec.insert_elements
    end
